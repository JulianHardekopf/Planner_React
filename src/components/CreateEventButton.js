...
const CreateEventButton = ({isopen, openform, setopenform}) => {
  return (
    <React.Fragment>
      ...
      {isopen ? <button onClick={() => !openform ? setopenform(!openform) : setopenform } className=' ml-4 p-2 rounded-full mt-20 outline-4 flex items-center bg-gray-50  hover:bg-gray-300 transition-all duration-500'>
        <BsPlusCircle />
